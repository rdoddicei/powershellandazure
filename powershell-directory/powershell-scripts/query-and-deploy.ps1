...
# Writing the logs to terraform variables
...
# Construct the terraform.tfvars content

$variablelogFilePath = "logs\variable-log.txt"
$variableLog | Set-Content -Path $variablelogFilePath -Encoding UTF8

# Read the logged file and format for terraform.tfvars
$logContent = Get-Content -Path $variablelogFilePath
$tfvarsContent = @()
...
$tfvarsContent -join "`n" | Set-Content -Path $tfvarsPath -Encoding UTF8

Write-Output "Updated terraform.tfvars file successfully."
...
Finding the $auth0ConnectionName connection in Auth0 tenant
...
$message =
"
Set $customerID RAMDB Database to FULL recovery model
...
#lets clean up the RAMDB database if needed
...
if([string]::IsNullOrEmpty($dropUsersList)) 
     { 
       $message = "No pre-existing users to remove from $bmramdb"
       Write-Output $message
       $message | Out-File $logfile -Append
     }
   else
   {
    $message =  "Users that exist on $bmramDB after initial restore: "
    Write-Output $message
    $message | Out-File $logfile -Append
    
    #$dropUsersList | Sort-Object -Descending  
    $dropUsersList | Format-Table | Out-String | Write-Host   
    $dropUsersList | Out-File $logfile -Append
    
   
   if($isDevelopmentEnvironment){
   $DropUsers = "Y"
   if (($result = Read-Host "To cleanup users from the RAMDB backup that is getting restored, do you want to drop existing database users? (i.e. Y or N) or press enter to accept default value `"$DropUsers`"") -eq '') {$DropUsers} else {$DropUsers= $result } 
   }else{
   $DropUsers = "Y"}

      if($DropUsers -eq "Y")
        {
         foreach ($dropUserList in $dropUsersList)
             {
             $dropUserList = $dropUserList.Name
	         $sql = "USE $($bmramDB); DROP SCHEMA IF EXISTS [$($dropUserList)]; DROP USER IF EXISTS [$($dropUserList)];";
	         exec-query -databaseConnection $applicationDatabaseConnection -sql $sql -continueOnError $continueOnError;
    
             $message =  "
             Removed user: $dropUserList from $bmramdb" 
             Write-Output $message
             $message | Out-File $logfile -Append
             }
        ...
        {$message = "
        Did NOT remove the database users!"
         Write-Output $message
         $message | Out-File $logfile -Append
        }
...
$message =
"
Initialize new Customer in BMRAMControl database
...
$message =
"
Update BMRAMControl with CustomerProps properties
...
$message =
"
Finding OSSIDs for BMQRAdmin and BMQRPM user access
...
$message =
"
Create our CLOUD AUTHENTICATION Auth Source
...
$message =
"
Create $customerID CLOUD AUTHENTICATION Auth Source
...
$message =
"
Create BMQRAdmin, BMQRPM, BMQRAnalysts personnel in BMRAM
...
$RSConfig = Get-RsDeploymentConfig –RsProjectFile $reportFilePath –ConfigurationToUse Debug
$RSConfig.OverwriteDatasets = $true
$RSConfig.OverwriteDataSources = $true
$RSConfig.TargetReportFolder = "$customerID/R4BPTemplate"
...
"publish R4BPTemplate images"
#publish images for R4BPTemplate
foreach($includeImage in $includeImages3){
$jpgLOC = "H:\CustomerDeployments\$customerID\ReportDeployments\R4BPTemplate\$includeImage"
...
<#
#launchIE
$launchIE = "http://$primarySvr/reports"
start $launchIE

Write-Output "Verify that the $customerID folder exists in SQL Server Reporting Services, see Edge window..."
#>
#get Reports
...
#######################################################################################################################
 ...
 $message =
"
Create Authorization (2 year) Grant
...
#creates User login records  (moved this to after the license is set)
 $message =
"
Create BMQRAdmin, BMQRPM, BMQRAnalysts user login records in BMRAM
...
Restore the RAMDB database on the secondary (using Replace)
...
Change database owner to RAMDatabaseOwner on RAMDB on the secondary
...
Restore the RAMDB database and log on the secondary (using NO RECOVERY)
...
Adding the $customer RAMDB to the AG
...
Bug 30658 (RamMobile) already placed, no need to run again. 
...
Beginning Incident 36128 Placement
...
Beginning Bug 133191 Fix (changeset 49325) Placement
...
Running 133191 Fix (changeset 49325) Placement on $bmramdb "
Write-Output $message
$message | Out-File $logfile -Append
...
$message = "
Completed 133191 Fix (changeset 49325) Placement on $bmramdb
...
#####INSTALL WEBAPI so that permissions are applied to the loginlessuser (even though the clean db has the webapi installed, I drop the loginlessuser and recreate it so I have to reintall to get permissions right
   ...
   $message | Out-File $logfile -Append
     ...
     Set-Location $PSScriptRoot
     <#
       $installLogfile = "installGraphQL.log"
       $installLogfileLoc = "$path\Scripts\installGraphQL.log"
       $destPath = "H:\CustomerDeployments\$customerID"
       Copy-Item $installLogfileLoc -Destination $destPath -Force
       $newName = "InstallGraphQL_duringProvision" + $versionFolder + "_" + $logDate + "_" + $customerID + ".txt"
       Rename-Item $destPath\$installLogfile -NewName $newName -Force
       #>
